...
ob_start();
session_start();
$UPDATE_info_member = json_decode($_REQUEST["UPDATE_info_member"]);

$data = $UPDATE_info_member;
 
try {
require_once("../Star_Way_Database.php");

	$sql="UPDATE info_member SET memPms=:memPms WHERE memNo=:memNo";
	$products = $pdo->prepare($sql);
	$products -> bindValue(":memNo", $data->memNo);
	$products -> bindValue(":memPms", $data->memPms); 
	$products -> execute();
